...
- ⏱ **Длительность текущей поездки** — `мм мин` или `Xч Yм` (напр. `47 мин`, `1ч 12м`). Старт — когда скорость > 3 км/ч 5 секунд подряд, конец — 3 минуты на нуле
...
- **X.X ↓** (18sp, цветной по тренду) — **средний расход текущей поездки**, кВт·ч/100км, со стрелкой тренда (см. ниже)
...
**Цифра справа от пробега (X.X)** — это средний расход **текущей поездки** в кВт·ч/100км: `кВт·ч, потраченные с начала поездки ÷ км × 100`.

Значение зависит от стадии поездки:

| Состояние | Что показано | Почему так |
|-----------|--------------|------------|
| Поездка не идёт | **baseline** (EMA истории) — без стрелки | Машина стоит — показываем ambient значение |
| Пробег < 1 км | **`—`** (прочерк) | Холодный старт, разгон с места дают 60–100 кВт·ч/100 — шум, цифре доверять нельзя |
| 1 км ≤ пробег < 2 км | **trip-avg** без стрелки | Уже честный средний по поездке, но мало данных для сравнения с baseline |
| Пробег ≥ 2 км | **trip-avg + стрелка тренда** | Можно сравнить твою текущую поездку с твоим обычным стилем |

**Стрелка тренда** сравнивает расход текущей поездки с вашим обычным стилем:
...
5. Скопируйте команду автозапуска D+ — вставьте в терминал DiLink (один раз)
...
- **Floating widget** — draggable 7-field overlay: SOC, range, consumption + trend arrow, trip time, cabin/battery temp, 12V
